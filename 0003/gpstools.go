...
func updateGBRstatus(update_GBR_id, update_CMD, update_GEO string, update_GBR_status int) {
	...
	*/
	...
	s_QUERY := "http://api-cs.ohholding.com.ua/api/set-status?status=" + update_CMD +
...
func getObjGeneral(objUIN int) string {
	...
	s_json = "{" + string(0x0D) + string(0x0A)
	s_json = s_json + getQuatedJSON("id", "0", 1) + "," + string(0x0D) + string(0x0A)
	s_json = s_json + getQuatedJSON("name", "CS SECURITY", 1) + "," + string(0x0D) + string(0x0A)
	s_json = s_json + getQuatedJSON("cmnd", "no_pult", 1) + string(0x0D) + string(0x0A)
	s_json = s_json + "}" + string(0x0D) + string(0x0A)
	return s_json
}

//------------------------------------------------------------------------------
func checkAlarmEnable(objUIN int) bool { //for getTokenList
	if len(gbrAlarmLast) < objUIN {
		for i := 0; i < len(alarmList_NUM_PULT); i++ {
			if alarmList_ID[i] == gbrAlarmLast[objUIN] {
				return true
