...
func decodeGpsJson(jsonIncoming string, conn *websocket.Conn) string {
	...
	if checkValidJson(jsonIncoming) == true { //Check valid data
		...
		if err != nil {
			defer recoveryAppFunction("Error decoding Json:" + jsonIncoming)
			panic(err)
		}
		...
		case "alarmget": //Receive alarm {"cmnd":"alarmget","id":"253805","name":"01","param":"-1110"}
			js_result = recAlarms(js_iden, js_cmnd, js_name, js_param)
		case "alarmstart": //GBR starts trip
		js_iden = "0"
			js_result = procAlarm(id_afterStart, js_cmnd, js_name, js_param)
		case "alarmpoint": //GBR at point
		js_iden = id_afterStart
...
func logoutGBR(userid, js_name, js_param string, conPosition int) string {
	...
	if gbrvalid > -1 && gbrvalid < len(gbrListID) {
		...
		gbrAlarmPult[gbrvalid] = ""
		gbrAlarmName[gbrvalid] = ""
		gbrAlarmAddr[gbrvalid] = ""
	}
...
func recAlarms(userid, js_cmnd, js_name, js_param string) string {
	...
	for i := 0; i < len(alarmList_ID); i++ {
		if alarmList_ID[i] == userid {
			s_card := ""
			if len(alarmList_HAVE_CARD[i]) < 10 { //No card enable
				s_card = "{" + getQuatedJSON("cmnd", "no_pult", 1) + ","
				s_card += getQuatedJSON("f_object_adress", alarmList_OBJ_ADDR[i], 1) + ","
				s_card += getQuatedJSON("f_object_name", alarmList_OBJ_NAME[i], 1) + ","
				s_card += getQuatedJSON("f_region", alarmList_OBJ_REGION[i], 1) + "}"
			} else {
				s_card = alarmList_HAVE_CARD[i]
			}
			s_json = s_card
			alarmList_WAS_SEND[i] = true
			for j := 0; j < len(gbrListID); j++ {
				if strconv.Itoa(gbrListID[j]) == js_name {
					gbrAlarmLast[j] = userid
					gbrAlarmPult[j] = alarmList_NUM_PULT[i]
					gbrAlarmName[j] = alarmList_OBJ_NAME[i]
					gbrAlarmAddr[j] = alarmList_OBJ_ADDR[i]
					gbrAlarmSend[j] = true
					gbrAlarmReceived[j] = true
					gbrAlarmType[j] = 0
					gbrAlarmWait[j] = 0
					gbrAlarmCanceled[j] = 0
					gbrAlarmCard[j] = s_card
				} else if gbrAlarmLast[j] == userid {
					//TODO: RESET THIS ALARM FROM OTHER GROUPS
					gbrAlarmLast[j] = ""
					gbrAlarmPult[j] = ""
					gbrAlarmName[j] = ""
					gbrAlarmAddr[j] = ""
					gbrAlarmSend[j] = false
			...
			fmt.Println("recAlarms", alarmList_HAVE_CARD[i])
		}
	}
	if s_json == "" {
		s_json = "{" + string(0x0D) + string(0x0A)
		s_json = s_json + getQuatedJSON("id", userid, 1) + "," + string(0x0D) + string(0x0A)
		s_json = s_json + getQuatedJSON("name", js_name, 1) + "," + string(0x0D) + string(0x0A)
		s_json = s_json + getQuatedJSON("cmnd", "getalarm", 1) + "," + string(0x0D) + string(0x0A)
		s_json = s_json + getQuatedJSON("param", "no_alarm", 1) + string(0x0D) + string(0x0A)
		s_json = s_json + "}" + string(0x0D) + string(0x0A)
	}
	return s_json
}
...
func procAlarmList() {
	for i := 0; i < len(alarmList_ID); i++ { //READ ALARMS
		if alarmList_WAS_SEND[i] == false && len(alarmList_ID[i]) > 0 { //CHECK WAS SEND
			for j := 0; j < len(gbrListID); j++ { //SEARCH GBR
				s_gbr := strconv.Itoa(gbrListID[j])                                                          //gbrListNUM[j]
				if len(s_gbr) > 0 && (alarmList_GBR_NAME[i] == s_gbr || alarmList_GBR_RESERVE[i] == s_gbr) { //GBR VALID
					if gbrAlarmType[j] < 2 { //WAS NOT CANCELED ALARM
						//fmt.Println("FIND GBR UIN", gbrListID[j], "NAME", gbrListNUM[j], "MAIN", alarmList_GBR_NAME[i], "RESERVED", alarmList_GBR_RESERVE[i], "ALARM", alarmList_ID[i], "CARD", alarmList_HAVE_CARD[i])
						isGBRcon := false
						gbrAlarmReceived[j] = false
						gbrAlarmType[j] = 1
						...
						if len(alarmList_NUM_PULT[i]) >= 0 && strings.Index(alarmList_HAVE_CARD[i], "no_pult") > 0 {
						...
						for k := 0; k < len(websock_gbr_name); k++ { //SEARCH GBR IN CONNECT LIST
							...
							if websock_gbr_uin[k] == gbrListID[j] { //GBR CONNECTED
								...
								if len(gbrAlarmCard[j]) < 10 { //No card enable
									gbrAlarmCard[j] = "{" + getQuatedJSON("cmnd", "no_pult", 1) + ","
									gbrAlarmCard[j] += getQuatedJSON("f_object_adress", alarmList_OBJ_ADDR[i], 1) + ","
									gbrAlarmCard[j] += getQuatedJSON("f_object_name", alarmList_OBJ_NAME[i], 1) + ","
									gbrAlarmCard[j] += getQuatedJSON("f_region", alarmList_OBJ_REGION[i], 1) + "}"
								}
								gbrAlarmWait[j] = 0
								alarmList_WAS_SEND[i] = true
						...
						if isGBRcon == false { //GBR NOT CONNECTED TO SOCKET
							if len(gbrAlarmTocken[j]) > 8 { //GBR TOCKEN IS VALID
								fmt.Println("GBR TOCKEN IS VALID 1", j, gbrAlarmTocken[j])
		...
		if len(gbrAlarmLast[i]) > 0 { //GROUP HAS ACTIVE ALARM
			...
			if gbrAlarmSend[i] == false && gbrAlarmType[i] < 2 { //ALARM NOT SENDED
				...
				if socketPosition > -1 { //SOCKET ENABLE
					...
					fmt.Println(time.Now(), "sendAlarmToGbr:", i, "GBR", gbrListNUM[i], "UIN", gbrListID[i], gbrAlarmLast[i])
					sendYeden(socketPosition, gbrAlarmCard[i])
					fmt.Println(time.Now(), socketPosition, gbrAlarmCard[i])
				} else if len(gbrAlarmTocken[i]) > 8 { //GBR FCM TOCKEN IS VALID
					fmt.Println(time.Now(), "sendAlarmPush:", i, "GBR", gbrListNUM[i], "UIN", gbrListID[i], gbrAlarmLast[i])
